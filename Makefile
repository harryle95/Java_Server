...
run_test: compile_src compile_test
	java -Djava.util.logging.config.file=src/config/logging.properties -javaagent:jar_files/intellij-coverage-agent-1.0.737.jar=src/config/config.args -jar $(JARDIR)/$(JUNITJAR) -cp $(JARFILES):$(TESTOUTDIR):$(OUTDIR) --scan-classpath
